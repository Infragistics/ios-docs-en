...
|metadata|
{
    "name": "igchartview-stacked-area-series",
    "controlName": ["IGChartView"],
    "tags": ["Charting","How Do I"],
    "guid": "9e0bdf53-4a83-4c00-8cca-4c3d7807e549",  
    "buildFlags": [],
    "createdOn": "2013-02-06T14:53:02.2523592Z"
}
|metadata|
////
...
** <<_Ref328076501,Stacked Area series summary>>
** <<_Ref326327824,Data requirements>>
** <<_Ref219345947,Data rendering rules>>

* <<_Ref328076508,Adding a Stacked Area Series to the  _IGChartView_   – Code Example>>
...
*Required –*  The bound data model must contain at least one numeric field, for rendering Stacked Area data and may contain an optional string for labels. For optimal viewing, we recommend using a data model containing two or more numeric data fields so that each point of the Stacked Area series has two or more stacked fragments.
...
* Every numeric column in the data model that you want rendered needs an  _IGStackedFragmentSeries_   added to the `series` collection property of the  _IGStackedAreaSeries_  .
...
* When rendering multiple series of the Stacked Area series type in layers, with each successive series rendered in front of the previous one in the `series` collection of the  _IGChartView_   control.

[[_Ref324842387]]
[[_Ref328076508]]
== Adding a Stacked Area Series to the  _IGChartView_   – Code Example

[[_Ref326327832]]

=== Description

The code below uses the link:igchartview-data-source-helpers.html[IGStackedSeriesDataSourceHelper] to supply randomly generated data to a Stacked Area series firstly adding to the  _IGChartView_   instance, and then adding the  _IGChartView_   as a subview of the current  _UIView_  .
...
public override void ViewDidLoad ()
{
      ...
      _chartView.AddStackedSeries(new Class(typeof(IGStackedAreaSeries)), "series", _stackedChartDataSourceHelper, "xAxis", "yAxis");
...
-(void)viewDidLoad
{
    ...
    [_chartView addStackedSeriesForType:[IGStackedAreaSeries class] usingKey:@"series" withDataSource:_stackedChartDataSourceHelper firstAxisKey:@"xAxis" secondAxisKey:@"yAxis"];    
