...
|metadata|
{
    "name": "igchartview-stacked-spline-area-series",
    "controlName": ["IGChartView"],
    "tags": ["Charting","How Do I"],
    "guid": "61d311a6-dce7-4a0c-8f75-817210fbd367",  
    "buildFlags": [],
    "createdOn": "2013-02-05T21:18:33.0624003Z"
}
|metadata|
////
...
** <<_Ref328076501,Stacked Spline Area series summary>>
** <<_Ref326327824,Data requirements>>
** <<_Ref219345947,Data rendering rules>>

* <<_Ref328076508,Adding a Stacked Spline Area Series to the  _IGChartView_   – Code Example>>
...
=== Stacked Spline Area series summary

Stacked Spline Area series belongs to a group of link:igchartview-category-series.html[Category Series] and renders using a collection of points connected by smooth curves of spline segments with the area below the spline filled in and stacked on top of each other. Each stacked fragment in the collection represents one visual element in each stack. Each stack can contain both positive and negative value, with positive values grouped on the positive side of the y-axis, and all negative values grouped on the negative side of the y-axis.

image::images/IGChartView_-_Stacked_Spline_Area_Series_1.png[]
...
*Required –*  The bound data model must contain at least one numeric field, for rendering Stacked Spline Area data and may contain an optional string for labels. For optimal viewing, we recommend using a data model containing two or more numeric data fields so that each point of the Stacked Spline Area series has two or more stacked fragments.
...
* Every numeric column in the data model that you want rendered needs an  _IGStackedFragmentSeries_   added to the `series` collection property of the  _IGStackedSplineAreaSeries_  .
...
* When rendering multiple series of the Stacked Spline Area series type in layers, with each successive series rendered in front of the previous one in the `series` collection of the  _IGChartView_   control.

[[_Ref324842387]]
[[_Ref328076508]]
== Adding a Stacked Spline Area Series to the  _IGChartView_   – Code Example

[[_Ref326327832]]

=== Description

The code below uses the link:igchartview-data-source-helpers.html[IGStackedSeriesDataSourceHelper] to supply randomly generated data to a Stacked Spline Area series firstly adding to the  _IGChartView_   instance, and then adding the  _IGChartView_   as a subview of the current  _UIView_  .
...
public override void ViewDidLoad ()
{
      ...
      _chartView.AddStackedSeries(new Class(typeof(IGStackedSplineAreaSeries)), "series", _stackedChartDataSourceHelper, "xAxis", "yAxis");
...
-(void)viewDidLoad
{
    ...
    [_chartView addStackedSeriesForType:[IGStackedSplineAreaSeries class] usingKey:@"series" withDataSource:_stackedChartDataSourceHelper firstAxisKey:@"xAxis" secondAxisKey:@"yAxis"];    
