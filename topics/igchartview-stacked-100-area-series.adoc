...
|metadata|
{
    "name": "igchartview-stacked-100-area-series",
    "controlName": ["IGChartView"],
    "tags": ["Charting","How Do I"],
    "guid": "4f667f70-6776-42de-a072-2857c55b050b",  
    "buildFlags": [],
    "createdOn": "2013-02-05T21:24:50.3934003Z"
}
|metadata|
////
...
** <<_Ref328076501,Stacked 100 Area series summary>>
** <<_Ref326327824,Data requirements>>
** <<_Ref219345947,Data rendering rules>>

* <<_Ref328076508,Adding a Stacked 100 Area Series to the  _IGChartView_   – Code Example>>
...
=== Stacked 100 Area series summary

Stacked 100 Area series belongs to a group of link:igchartview-category-series.html[Category Series] rendered as a collection of points connected by line segments with the area below the line filled in and stacked on top of each other. Each stacked fragment in the collection represents one visual element in each stack. Each stack can contain both positive and negative value, with positive values grouped on the positive side of the y-axis, and all negative values grouped on the negative side of the y-axis. The Stacked 100 Area series is identical to the Stacked Area series in all aspects except in their treatment of the y-axis values. Instead of presenting a direct representation of the data, the Stacked 100 Area series presents data in terms of percent of the sum of all values in a data point.

image::images/IGChartView_-_Stacked_100_Area_Series_1.png[]
...
*Required –*  The bound data model must contain at least one numeric field, for rendering Stacked 100 area data and may contain an optional string for labels. For optimal viewing, we recommend using a data model containing two or more numeric data fields so that each point of the Stacked 100 Area series has two or more stacked fragments.
...
* Every numeric column in the data model that you want rendered needs an  _IGStackedFragmentSeries_   added to the `series` collection property of the  _IGStacked100AreaSeries._  
...
* When rendering multiple series of the Stacked 100 Area series type in layers, with each successive series rendered in front of the previous one in the `series` collection of the  _IGChartView_   control.

[[_Ref324842387]]
[[_Ref328076508]]
== Adding a Stacked 100 Area Series to the  _IGChartView_   – Code Example

[[_Ref326327832]]

=== Description

The code below uses the link:igchartview-data-source-helpers.html[IGStackedSeriesDataSourceHelper] to supply randomly generated data to a Stacked 100 Area series firstly by adding to the  _IGChartView_   instance, and then adding the  _IGChartView_   as a subview of the current UIView.
...
public override void ViewDidLoad ()
{
      ...
      _chartView.AddStackedSeries(new Class(typeof(IGStacked100AreaSeries)), "series", _stackedChartDataSourceHelper, "xAxis", "yAxis");
...
-(void)viewDidLoad
{
    ...
    [_chartView addStackedSeriesForType:[IGStacked100AreaSeries class] usingKey:@"series" withDataSource:_stackedChartDataSourceHelper firstAxisKey:@"xAxis" secondAxisKey:@"yAxis"];    
